...
def main(train_file, dev_file, target_dir, 
         ...
         max_grad_norm=10.0,
         checkpoint=None):
    distilbert_tokenizer = DistilBertTokenizer.from_pretrained('adamlin/bert-distil-chinese', do_lower_case=True)
    ...
    train_data = DataPrecessForSentence(distilbert_tokenizer, train_file)
    train_loader = DataLoader(train_data, shuffle=True, batch_size=batch_size)
    print("\t* Loading validation data...")
    dev_data = DataPrecessForSentence(distilbert_tokenizer,dev_file)
    ...
    # -------------------- Training epochs ------------------- #
    print("\n", 20 * "=", "Training DistilBertModel model on device: {}".format(device), 20 * "=")
    patience_counter = 0
