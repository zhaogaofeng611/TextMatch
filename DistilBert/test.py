...
from model import DistilBertModelTest
...
def main(test_file, pretrained_file, batch_size=32):
    ...
    distil_tokenizer = DistilBertTokenizer.from_pretrained('models/vocabs.txt', do_lower_case=True)
    ...
    test_data = DataPrecessForSentence(distil_tokenizer, test_file)
    test_loader = DataLoader(test_data, shuffle=True, batch_size=batch_size)
    print("\t* Building model...")
    model = DistilBertModelTest().to(device)
